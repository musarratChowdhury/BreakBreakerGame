...
function GameAreaStart(){
    ...
    myObj.start();
    paddle = new rectDrawTool(canvas_W/2-50,canvas_H-50,PADDLE_WIDTH,PADDLE_HEIGHT,"./svg/paddle.jpg");
...
function updateGameArea(){
    scoreText = new textBuilder(canvas_W/2-50,40,"DarkBlue",'Score : '+score);
    levelBanner = new textBuilder(canvas_W-130,40,"DarkBlue","Level : "+level);
    if(showingHoldScreen){//Holdin the ball on the paddle untill a mouse click is triggered!!
        if(lives<1){
            myObj.clear();
            endmsg = new textBuilder(canvas_W/2-150,canvas_H/2-20,'DarkBlue','GameOver! Your Score : '+score);
            endmsg.draw();
            myObj.stop();
            return;
        }
   ...
   //~Drawn the bricks///////////////////
    ...
    for(let i = 0;i<bricks.length;i++){//////////////////checking collision with the bricks////////////////////
        ...
        if(ball.crashWith(bricks[i])){
           ...
           if(bricks[i].flag>1){
             ...
             if(bricks.length<1){//checking if the length of the bricks array gets reduced to 0;///
                 levelchange=true;
                 level++;
             }
            // },10)
...
class ballpainter{
    ...
    move(){///............normal movement of the ball.....................///
        if(ball.up){        
            ball.y-=ball.ySpeed;
        }
        else{           
    ...
    chkCollsionWithWalls(){//......................CHK COLLISON WITH THE WALLS....///
        ...
        if(ball.y+ball.rad>=myObj.canvas.height){///neeed to lose life here,,,,,,,,,,,,,,,,!!!!!!!
    ...
    chkCollsionWithPaddle(){///...........................HIT THE PADDLE<<<<<<<<<<<<<<<<<<<<
        
        if(ball.y+ball.rad>paddle.y&&ball.y+ball.rad<paddle.y+PADDLE_HEIGHT){
            if(ball.x+ball.rad>paddle.x&&ball.x-ball.rad<paddle.x+PADDLE_WIDTH){
